...
export default function ReportForm({ onAddReport, onCancel, selectedCoords }: ReportFormProps) {
  ...
  return (
    <div className="bg-zinc-950 border border-white/10 rounded-3xl p-6 shadow-2xl h-full flex flex-col justify-between" id="report-form-panel">
      ...
      <div>
        <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-5">
          ...
          <div className="flex gap-1.5">
            <span className={`w-6 h-1.5 rounded-full ${step >= 1 ? 'bg-brand-gold' : 'bg-zinc-900'}`} />
            <span className={`w-6 h-1.5 rounded-full ${step >= 2 ? 'bg-brand-gold' : 'bg-zinc-900'}`} />
            <span className={`w-6 h-1.5 rounded-full ${step >= 3 ? 'bg-brand-gold' : 'bg-zinc-900'}`} />
          </div>
        </div>
        ...
        {step === 1 && (
          <div className="space-y-4 animate-fade-in">
            <div className="bg-zinc-950 border border-white/10 rounded-2xl p-4 flex gap-4 items-center">
              <div className="w-10 h-10 rounded-xl bg-brand-gold/10 flex items-center justify-center text-brand-gold border border-brand-gold/35 flex-shrink-0">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full bg-zinc-950 border border-white/10 rounded-xl px-4 py-3 text-slate-100 text-xs focus:outline-none focus:ring-1 focus:ring-brand-gold"
                required
              />
            </div>
            ...
            <div className="pt-4 border-t border-white/10">
              ...
              {isAnalyzing ? (
                <div className="bg-zinc-950 border border-brand-gold/35 rounded-2xl p-6 text-center flex flex-col items-center justify-center space-y-3 shadow-lg">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button
                    ...
                    className="flex flex-col items-center p-3 bg-zinc-950 hover:bg-zinc-900 border border-white/10 hover:border-brand-gold/30 rounded-xl text-center group transition-all"
                  >
        ...
        {step === 2 && (
          <div className="space-y-4 animate-fade-in">
            {aiAnalysisResult && (
              <div className="bg-brand-gold/10 border border-brand-gold/30 rounded-2xl p-5 space-y-4 relative overflow-hidden shadow-lg">
                <div className="absolute top-4 right-4 bg-brand-gold/15 border border-brand-gold/30 text-brand-gold font-mono text-[9px] px-2.5 py-0.5 rounded-full font-black uppercase flex items-center gap-1">
                ...
                <div className="grid grid-cols-2 gap-3 border-t border-white/10 pt-3.5">
                  <div>
                    <span className="text-[9px] font-black uppercase tracking-wider text-slate-500">Confidence</span>
                    <div className="text-sm font-mono font-black text-brand-gold mt-0.5 font-bold">{(aiAnalysisResult.confidence * 100).toFixed(0)}%</div>
                  </div>
                  <div>
            ...
            <div className="bg-zinc-950 p-4 rounded-2xl border border-white/10 flex items-center justify-between shadow-inner">
              <div>
              ...
              <label className="bg-zinc-900 hover:bg-zinc-800 text-slate-200 cursor-pointer text-xs font-semibold px-4 py-2.5 rounded-xl border border-white/10 flex items-center gap-1.5 transition-all">
          ...
          <div className="space-y-4 animate-fade-in">
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-xl text-center border transition-all flex flex-col items-center justify-center ${
                      hazardType === type
                        ...
                        : 'border-white/10 bg-zinc-950 text-slate-400 hover:bg-zinc-900'
                    }`}
                  >
                ...
                {Object.entries(SEVERITY_CONFIG).map(([level, config]) => (
                  <button
                    ...
                    className={`py-2 rounded-xl text-center border text-[10px] font-black uppercase tracking-wider transition-all ${
                      severity === level
                        ? 'border-white/20 shadow-lg text-white font-black'
                        : 'border-transparent bg-zinc-950 text-slate-500 hover:bg-zinc-900/50'
                    }`}
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full bg-zinc-950 border border-white/10 rounded-xl px-4 py-3 text-slate-100 text-xs focus:outline-none focus:ring-1 focus:ring-brand-gold h-24 resize-none"
                required
              />
            </div>
      ...
      <div className="flex items-center gap-3 border-t border-white/10 pt-5 mt-6">
        <button
          type="button"
          onClick={onCancel}
          className="flex-1 bg-zinc-900 hover:bg-zinc-800 text-slate-300 text-xs font-black uppercase tracking-wider py-3.5 rounded-xl transition-all cursor-pointer"
        >
          Cancel
        </button>
        ...
        {step > 1 && (
          <button
            ...
            className="p-3.5 bg-zinc-900 hover:bg-zinc-800 text-slate-300 rounded-xl transition-all cursor-pointer"
          >
