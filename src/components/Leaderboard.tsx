  ...
  return (
    <div className="space-y-6" id="leaderboard-panel">
      ...
      <div className="bento-card-gradient border border-white/5 rounded-3xl p-6 shadow-xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="bg-brand-gold/10 border border-brand-gold/20 text-brand-gold font-mono text-[10px] uppercase tracking-wider px-4 py-2.5 rounded-2xl flex items-center gap-2 font-black shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {users.slice(0, 3).map((user, idx) => {
          const podiumColors = [
            { bg: 'from-brand-gold/25 to-zinc-950/40 border-brand-gold/30', medal: '🥇', text: 'text-brand-gold', label: '1st Champion' },
            { bg: 'from-slate-400/25 to-zinc-950/40 border-slate-400/30', medal: '🥈', text: 'text-slate-300', label: '2nd Contributor' },
            { bg: 'from-amber-700/25 to-zinc-950/40 border-amber-700/30', medal: '🥉', text: 'text-amber-600', label: '3rd Guard' }
          ][idx];
          ...
          return (
            ...
            >
              ...
              <div className="w-14 h-14 rounded-full bg-zinc-950 flex items-center justify-center text-3xl shadow-lg border border-white/5">
                {podiumColors.medal}
              </div>
              <div>
                <h3 className="font-display font-black text-base text-slate-100">{user.name}</h3>
                <span className="text-[9px] bg-white/5 border border-white/5 text-slate-300 px-3 py-1 rounded-full font-mono mt-2 inline-block uppercase tracking-wider">
                  {user.role} • {user.district.split(' ')[0]}
                </span>
              </div>

              <div className="grid grid-cols-2 gap-4 w-full border-t border-white/5 pt-4 text-[9px] font-black uppercase tracking-wider text-slate-500">
                <div>
      ...
      <div className="bg-zinc-900 border border-white/5 rounded-3xl overflow-hidden shadow-xl">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-[#0a0a0b]/50 text-slate-500 uppercase tracking-wider text-[9px] font-black border-b border-white/5">
              <tr>
                ...
                return (
                  <tr
                    ...
                    className={`hover:bg-white/5 transition-colors ${
                      isCurrentUser ? 'bg-brand-gold/5 hover:bg-brand-gold/10 border-l-4 border-l-brand-gold' : ''
                    }`}
                  >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-xl bg-zinc-950 flex items-center justify-center text-xs border border-white/5 ${isCurrentUser ? 'text-brand-gold font-bold' : 'text-slate-400'}`}>
